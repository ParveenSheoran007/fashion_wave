...
class ProductService {
  ...
  static Future<List<ProductModel>> fetchProducts() async {
    ...
    if (response.statusCode == 200) {
      String body = response.body;
      List<Map<String, dynamic>> listMap = List<Map<String, dynamic>>.from(jsonDecode(body));
      List<ProductModel> productList = listMap.map((map) => ProductModel.fromMap(map)).toList();
      return productList;
    } else {
      throw 'Something went wrong';
    }
  ...
  static Future<ProductModel> deleteProduct(String productId) async {
    ...
    String body = response.body;
    var json = jsonDecode(body);
    ProductModel deleteProductResponse = ProductModel.fromJson(json);
